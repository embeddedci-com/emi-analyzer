...
export function CableEmissionPanel({ api, runId, projectId, manifest }: CableEmissionPanelProps) {
  ...
  const [data, setData] = useState<{ transfers: CableTransfer[]; antennas: CableAntenna[] } | null>(
    null,
  )
  ...
  const composed = useMemo(() => {
    ...
    const transfer = data.transfers.find((t) => t.ref === ref)
    const antenna = data.antennas.find((a) => a.ref === ref)
    if (!transfer || !antenna) return null
    try {
      const parsed = parseDriverDocument(selectedDriver.document)
      const resolved = resolveDriver(parsed, transfer.frequencies_hz)
      const emission = composeEmission(transfer, antenna, sourceVolts(resolved))
      // The driver's own reasons for saying nothing are more specific than the composition's,
      // so they win where both have an opinion about the same frequency.
      for (const [f, why] of resolved.undriven) emission.undriven.set(f, why)
      return { parsed, emission, error: null as string | null }
    } catch (e) {
      return { parsed: null, emission: null, error: (e as Error).message }
    }
  }, [data, ref, selectedDriver])

...
KiCad's PCM reads a repository from three files: repository.json, which points at
packages.json and resources.zip with their hashes. packages.json lists every package and
every version with a download URL and hash; resources.zip holds an icon per package. This
script produces all of it:
    ...
    2. packages.json in the repository directory, with this version added (or replaced) and
       every earlier version kept
    3. resources.zip and repository.json, re-hashed

The archive is small -- Python, two manifests and five icons -- because the plugin ships no
engine of its own. What does the work is the EMI Analyzer desktop app, which the user
installs separately.

The repository is the same one the other EmbeddedCI plugins are published from
(embeddedci-com/kicad-plugins), so a user who has added it once gets this plugin too. Only
the index files live there; the plugin's source and its archive stay in this repository.
...
Standard library only. Usage (normally through `make pcm-release`):
    ...
    python scripts/pcm_release.py --version 0.1.0 --repo ../../kicad-plugins \\
        --download-url https://github.com/.../releases/download/kicad-plugin-v0.1.0/x.zip \\
...
# What goes into plugins/. Everything else in the folder -- tests, scripts, the package
# description, caches -- stays out.
INCLUDE = [
    "plugin.json",
    "requirements.txt",
    "README.md",
...
# Files taken from the repository rather than from the plugin folder, so there is one copy
# of them: {name in the archive: path from the repository root}.
FROM_REPO = {"LICENSE": "LICENSE"}
EXCLUDE_PARTS = {"__pycache__", ".pytest_cache", ".DS_Store"}

# A fixed timestamp inside the archive, so the same inputs give the same bytes and the same
# hash.
...
def check_complete(plugin: Path, repo_root: Path) -> None:
    """Refuse to package a plugin that is missing something it needs to run."""
    missing = [name for name in INCLUDE if not (plugin / name).exists()]
    missing += [src for src in FROM_REPO.values() if not (repo_root / src).is_file()]
    if missing:
        raise ReleaseError("the plugin is not complete; missing:\n  " + "\n  ".join(missing))


def declared_version(plugin: Path) -> str:
    """__version__ as the package itself states it."""
    text = (plugin / "emi_analyzer" / "__init__.py").read_text(encoding="utf-8")
    for line in text.splitlines():
        if line.startswith("__version__"):
            return line.split("=", 1)[1].strip().strip("\"'")
    raise ReleaseError("emi_analyzer/__init__.py does not declare __version__")
...
def build_archive(
    plugin: Path, desc: Dict[str, Any], version: str, status: str, out: Path, repo_root: Path
) -> Dict[str, int]:
    ...
    with zipfile.ZipFile(out, "w") as z:
        for f in _files(plugin):
            ...
            data = f.read_bytes()
            install += len(data)
            _add(z, "plugins/" + rel, data)
        for name, src in sorted(FROM_REPO.items()):
            data = (repo_root / src).read_bytes()
            install += len(data)
            _add(z, "plugins/" + name, data)
...
def merge_packages(existing: Dict[str, Any], desc: Dict[str, Any], version: Dict[str, Any]) -> Dict[str, Any]:
    ...
    The package's text is refreshed from pcm.json; its other versions are kept, newest
    first, so a user can still see and roll back to them. Publishing a version that is
    already there replaces it -- a re-release of a broken zip. Other packages in the
    repository, such as the trace length analyzer, are left exactly as they were.
    """
...
def release(
    ...
    declared = declared_version(plugin)
    if declared != version:
        raise ReleaseError(
            f"emi_analyzer/__init__.py says __version__ = {declared!r}; bump it to {version!r} first"
        )
    desc = json.loads((plugin / "pcm.json").read_text(encoding="utf-8"))
    check_complete(plugin, repo_root)

    repo.mkdir(parents=True, exist_ok=True)
    archive_dir = archive_dir or (repo_root / "dist" / "pcm")
    archive = archive_dir / f"emi-analyzer-kicad-plugin-{version}.zip"
    sizes = build_archive(plugin, desc, version, status, archive, repo_root)
    ...
    resources = write_resources(repo, packages, {desc["identifier"]: (plugin / "icons" / "emi-64.png").read_bytes()})
...
def main(argv: List[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--repo-url", required=True, help="the URL repository.json, packages.json and resources.zip are served under"
    )
    ...
    try:
        out = release(
            args.version, args.repo, args.download_url, args.repo_url, args.status, archive_dir=args.archive_dir
        )
    except ReleaseError as e:
        print(f"pcm_release: {e}", file=sys.stderr)
        return 1
    v = out["version"]
    print(f"archive     {out['archive']}  ({v['download_size'] / 1e3:.0f} kB, sha256 {v['download_sha256'][:12]}…)")

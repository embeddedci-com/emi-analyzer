-- Migration 1: the EMI Analyzer schema for the local (desktop) app, as v0.1.0 shipped it.
--
-- The same tables as server/emi/schema.sql, translated to SQLite. Keep the two in step: a column
-- added there and not here compiles, and then fails at the first INSERT of the local app. A
-- change is a new numbered file in this directory, never an edit to this one (see migrate.go).
--
-- Every statement here is IF NOT EXISTS, and must stay that way: databases created before the
-- migration ladder existed have these tables already and user_version 0, so this step runs on
-- them too and has to change nothing.
--
...
--   * foreign keys are switched on per connection by OpenSQLite, not here: the pragma does
--     nothing inside the transaction a migration runs in
